...
// Check if array is sorted acording to passed in predicate
template <class T>
...
template <class T>
int partition(int* a, int lo, int hi, std::function<bool(const T&, const T&)> predicate) {
...
template <class T>
void quicksort(int* a, int lo, int hi, const std::function<bool(const T&, const T&)>& predicate) {
  if ( lo < hi ) {
    int pivot = partition(a, lo, hi, predicate);

    quicksort(a, lo, pivot - 1, predicate);
    quicksort(a, pivot + 1, hi, predicate);
  }
}
