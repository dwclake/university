...
struct Array {
    ...
    Array(const int& size, const t& init);
    ...
    void set_predicate(const std::function<bool(const t&, const t&)>& predicate) { this->predicate = predicate; }
...
// Constructor
template <class t>
Array<t>::Array(const int& size, const t& init) {
...
template <class t>
void Array<t>::sort() {
    quicksort(this->array, 0, this->size - 1, this->predicate);
}
...
void Array<t>::insert(const t& value) {
    ...
    if ( !sorted(this->array, this->size, this->predicate) ) {
        this->sort();
    }

    // Find the first element where predicate returns true, which will be the idx to insert
    int in_idx = first_match(this->array, this->size, value, this->predicate); 
...
void Array<t>::delete_matches(const t& value) {
    ...
    if ( matches.get_size() == 0 ) return;

    // Assign new size and allocate new memory
    int new_size = old_size - matches.get_size();
    this->array = new t[new_size];
    this->size = new_size;
    this->capacity = new_size;
    ...
    for ( int i = 0; i < matches.get_size(); i++ ) {
